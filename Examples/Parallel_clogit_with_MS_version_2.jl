...
dQdP_g = getGroupdQdP( xstar, df0, clm, Q_g, P_g,  :cost_div_Y, :owner, pos_PdivY, OWN.IND) 
DR_g = getGroupDiversionRatioMatrix( xstar, df0, clm, Q_g, :cost_div_Y, :owner, pos_PdivY, OWN.IND) 

AdQdP_g = getApproxGroupdQdP(AD, OWN.IND, clm.opts[:PdivY])
ADR_g = getApproxGroupDiversionRatioMatrix( AD , OWN.IND, clm.opts[:PdivY])
...
FPMS_FOC0(x) = FPMS_FOC(zeros(J), xstar, df1, clm, MC, OWN.MAT, x, 
						:cost_div_Y, :cost, :invY, pos_PdivY, PARALLEL_FLAG)
...
# Ben's method: MC + zeta(p) where zeta(p) = invL * (OMEGA .* Γ) * (P - MC) - invL * Q 
FPMS_FOC1(x) = FPMS_FOC(zeros(J), xstar, df1, clm, MC, POST_OWN.MAT, x, 
						:cost_div_Y, :cost, :invY, pos_PdivY, PARALLEL_FLAG)
...
cl1 = new_clogit_data(df1, clm, P1, :cost_div_Y, :cost, :invY)
