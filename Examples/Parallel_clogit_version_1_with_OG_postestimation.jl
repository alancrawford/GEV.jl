...
dsdP = dQdP ./ length(AD)
DR = spgetPriceDiversionRatioMatrix(AD0, J)
...
DR_ig = spgetPriceDiversionRatioMatrix(AD0, J, inside_good_idx)
...
dQdP_g = spgetGroupdQdX( xstar, df0, clm, J, Q_g, P_g,  :cost, :owner, pos_price, OWN.IND) 
AdQdP_g = spgetGroupdQdP(AD0, J, OWN.IND)

DR_g = spgetGroupDiversionRatioMatrix( xstar, df0, clm, J, Q_g, :cost, :owner, pos_price, OWN.IND) 
ADR_g = spgetGroupPriceDiversionRatioMatrix( AD0 , J, OWN.IND)
...
FOC0(x) = spFOC(zeros(NumInsideProds), xstar, df1, clm, MC_ig, PRE_OMEGA, x, J, inside_good_idx, :cost, pos_price, PARALLEL_FLAG)
...
FPMS_FOC0(x) = spFPMS_FOC(zeros(NumInsideProds), xstar, df1, clm, MC_ig, PRE_OMEGA, x, J, inside_good_idx, :cost, pos_price, PARALLEL_FLAG)
...
PARALLEL_FLAG = false
FOC1(x) = spFOC(zeros(NumInsideProds), xstar, df1, clm, MC_ig, POST_OMEGA, x, J, inside_good_idx, :cost, pos_price, PARALLEL_FLAG)

# Solve for post-merger prices (start from pre-merger)
post_res = nlsolve(FOC1, P_ig)

# Ben's method: MC + zeta(p) where zeta(p) = invL * (OMEGA .* Γ) * (P - MC) - invL * Q 
FPMS_FOC1(x) = spFPMS_FOC(zeros(NumInsideProds), xstar, df1, clm, MC_ig, POST_OMEGA, x, J, inside_good_idx, :cost, pos_price, PARALLEL_FLAG)
...
cl1 = new_clogit_data(df1, clm, P1_CWinput, :cost)

CW0 = getCW(AggregateDemand(xstar, df0, cl0, pos_price))
CW1 = getCW(AggregateDemand(xstar, df1, cl1, pos_price))
CW_CHANGE = CW1/CW0 - 1
