...
dQdP_g = getGroupdQdP( xstar, df0, clm, Q_g, P_g,  :cost, :owner, pos_price, pos_invY, OWN.IND) 
DR_g = getGroupDiversionRatioMatrix( xstar, df0, clm, Q_g, :cost, :owner, pos_price, pos_invY, OWN.IND) 
...
FPMS_FOC0(x) = FPMS_FOC(zeros(J), xstar, df1, clm, MC, OWN.MAT, x, :cost, pos_price, pos_invY, PARALLEL_FLAG)
...
PARALLEL_FLAG = false
FOC1(x) = FOC(zeros(J), xstar, df1, clm, MC, POST_OWN.MAT, x, :cost, pos_price, pos_invY, PARALLEL_FLAG)

# Solve for post-merger prices (start from pre-merger)
post_res = nlsolve(FOC1, P_init)

# Ben's method: MC + zeta(p) where zeta(p) = invL * (OMEGA .* Γ) * (P - MC) - invL * Q 
FPMS_FOC1(x) = FPMS_FOC(zeros(J), xstar, df1, clm, MC, POST_OWN.MAT, x, :cost, pos_price,  pos_invY, PARALLEL_FLAG)
...
cl1 = new_clogit_data(df1, clm, P1, :cost)
CW1 = getCW(AggregateDemand(xstar, df1, cl1, pos_price, pos_invY))
CW_CHANGE = CW1/CW0 - 1
